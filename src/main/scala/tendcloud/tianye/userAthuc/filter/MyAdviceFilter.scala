...
class MyAdviceFilter extends AdviceFilter{
  override def preHandle(request: ServletRequest, response: ServletResponse): Boolean = super.preHandle(request, response)

  override def postHandle(request: ServletRequest, response: ServletResponse): Unit = super.postHandle(request, response)
